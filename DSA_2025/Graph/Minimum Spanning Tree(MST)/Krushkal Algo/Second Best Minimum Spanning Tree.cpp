...
public:
    ...
    // ----------------------------------------------------------
    // Build MST but SKIP exactly one edge (used for 2nd MST)
    // ----------------------------------------------------------
    int buildMST(int V, vector<vector<int>>& edges, int skipIndex) {
        DSU dsu(V);
        int sum = 0, usedEdges = 0;
        ...
        return (usedEdges == V - 1 ? sum : INT_MAX);
    }

    // ----------------------------------------------------------
    // FIRST MST and record index of chosen edges
    // ----------------------------------------------------------
    int firstMST(int V, vector<vector<int>>& edges, vector<int> &picked) {
        DSU dsu(V);
        int sum = 0, used = 0;
    ...
    int secondMST(int V, vector<vector<int>> &edges) {
        ...
        for (int idx : picked) {
            int mst2 = buildMST(V, edges, idx);
            if (mst2 > mst1) secondBest = min(secondBest, mst2);
        }
