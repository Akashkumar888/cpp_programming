...
public:
  int solve(int i,int j,string &s1,string &s2,vector<vector<int>>&dp){
    ...
    int minDistance(string word1, string word2) {
        ...
        return  n+m - 2*solve(n,m,word1,word2,dp);
    }
};
...
public:
    int minDistance(string word1, string word2) {
        int n=word1.size();
        int m=word2.size();
        vector<vector<int>>dp(n+1,vector<int>(m+1,0)); // beacuse dp[n][m] that why [n+1][m+1] use 
...
public:
    int minDistance(string word1, string word2) {
        ...
        for(int i=1;i<=n;i++){
            for(int j=1;j<=m;j++){
                if(word1[i-1]==word2[j-1])temp[j]= 1+prev[j-1];
...
// it is also same as shoretest common supersequences 